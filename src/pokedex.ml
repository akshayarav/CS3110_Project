...
let json_to_base_pokemon json =
  {
    ...
    moves =
      json |> member "moves" |> to_list |> List.map to_string
      |> List.map string_to_move;
    learnable_moves =
      json |> member "learnable_moves" |> to_list
      |> List.map (fun jm ->
             ( jm |> member "level" |> to_int,
               jm |> member "move" |> to_string |> string_to_move ));
    evolution = None;
  }
...
let add_evolution_details base_pokemon all_pokemon json =
  ...
  | ev ->
      ...
      let evolved_form =
        List.find (fun p -> p.name = evolved_name) all_pokemon
      in
...
let pokemons_from_json file =
  ...
  List.map2
    (fun base_pokemon json ->
      add_evolution_details base_pokemon base_pokemon_list json)
    base_pokemon_list json_list
