...
let () =
  ...
  Hashtbl.add type_ineffectiveness Bug
...
let create base level =
  {
    base;
    hp = base.max_hp;
    current_max_hp = base.max_hp;
    (* Initialize current max HP *)
    level;
    xp = (0, 10 * level);
    feint = false;
    damage_modifier = 1.;
...
let create_random base level =
  let min_hp = base.max_hp / 2 in
  (* Minimum HP is half of the max HP *)
...
let attack move opponent_pokemon player_pokemon =
  ...
  let new_hp =
    opponent_pokemon.hp
    - int_of_float (float_of_int move.damage *. damage_multiplier)
  in
  if new_hp <= 0 then (
...
let level_up pokemon =
  let rec level_up_helper pokemon =
    ...
    if current_xp >= next_level_xp && pokemon.level < max_level then
      ...
      let new_max_hp = pokemon.current_max_hp + (new_level * 2) in
      (* Updated to use current_max_hp *)
      ...
      let updated_pokemon =
        {
          ...
          hp = (if pokemon.hp > 0 then new_max_hp else pokemon.hp);
          (* Heal if not feinted *)
